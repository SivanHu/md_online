...
Detect the current Git repository name as the project. Pass `--project` only when that name is misleading. Choose a path relative to that project's directory, such as `notes/report.md` or `weekly/2026-W29.md`. Preserve the source filename under `documents/` by default.
...
node scripts/upload-markdown.mjs \
  --input <local-file.md> \
  --path <project-relative-path.md>
```

Add `--project <name>` when necessary. Add `--overwrite` only with explicit authorization to replace an existing document. Omit `--path` to upload to `projects/<project>/documents/<source-name>`. Use `--dry-run` to validate and display the destination without writing.
